...
NEXT STEP:          OPERATE. P6 is DONE (2026-08-14): `FinEventsDailyRunner` is
                    ...
                    parked; its prerequisites are listed there. ONE schedule
                    correction the same evening (not a lever): GDELT publishes
                    yesterday's file at 07:00 GMT daily, and 11:00 IST is
                    05:30 GMT — 90 min too early, so the brief was a day stale
                    on news by construction. The task now fires 12:45 IST
                    (07:15 GMT) from 2026-08-18; the digest follows at ~13:10;
                    the fetcher no longer blanks the window on unpublished
                    head days. From here: no reseals, ever; the scheduler
                    seals, the digest reports, the builder watches. Review the
                    week's record on or after Monday 2026-08-24 before deciding
                    what to change. The AWS ladder resumes after the POC verdict.
                    2026-08-18 REVIEW (docs/analysis/poc-review-2026-08-18.md):
                    after two strict runs the agent and the base rates are
                    indistinguishable (n=2); every number recomputed twice;
                    18 methodology findings upheld under adversarial review —
                    five DEFECTS (D1 knowledge-time leak for USD/INR & WTI,
                    D2 t+5 self-grade in the brief uses the 1-day move, D3 a
                    partial CBR fetch could seal a backdated record, D4 a
                    mid-run crash loses paid bets, D5 the curator cannot
                    retire its last lesson) recommended for immediate fix,
                    ...
                    2026-08-19 (later): L1 DONE on the builder's go — ADR-0058
                    accepted, REQ-1304 added. The reasoning rung now ships with
                    controls: llm_noevents (the brief minus events — the
                    events->prices thesis isolated), llm_blind (minus the other
                    methods' bets, the live record and the offline ladder — the
                    anchoring control ADR-0029 asked for), and the deterministic
                    momentum_climo (climatology with the recent drift added in
                    sigma units) as a free bar for the momentum story. Purely
                    additive: llm_raw's brief is byte-identical, pinned by a
                    test, so the record already accruing is undisturbed. The
                    primary comparison (llm_raw - llm_noevents, paired, NW
                    lag h-1, day-clustered, ~60 days) is pre-registered in the
                    ADR before the first control seals. They start their own
                    clock at tomorrow's 12:45 run. ~12 model calls/day, up from
...
| **P8c** | **The reasoning rung — Strands (ADR-0057), key-ready** | ✅ built, awaiting only `OPENAI_API_KEY` — `strands-agents[openai]` added per ADR-0057; `scripts/poc_reasoning.py` assembles the deterministic brief (today's geometry, every method's sealed bet incl. controls, live record, offline ladder, event shortlist — no URLs, no article text), one bounded structured-output turn, full transcript to gitignored `data/reasoning/` with SHA-256 hashes in the seal (REQ-1302). Keyless runs skip visibly; `--show-brief gold` prints the exact prompt (6.4 KB today). Gold-first; `llm_raw` seals with the next fresh print once the key lands. 7 tests. Forward-only: no offline history for this rung, ever |
| **P8d** | **The mini-wiki memory** — [design](design/poc-mini-wiki.md) | ✅ done — `scripts/poc_wiki.py`: statistics computed by code (`seeded`/`observed`, never averaged), lessons model-written but code-enforced (≤15, cited against real evidence dates, over-long/uncited/mis-cited dropped with named reasons, empty proposals keep rather than wipe), pages versioned append-only. `llm_raw` + `llm_mem` seal daily; their paired difference is memory's measured value. 12 wiki tests. **First live day (2026-08-13→14): 3 horizons scored (TimesFM won gold's), and GPT-5.6's first sealed bets landed** — rationale on record: events noisy, track short, stay near the bar. Shakedown found and fixed two real defects: 4dp rounding broke the RPS sum contract (round-with-repair at seal + normalise for legacy rows), and Strands' `OpenAIModel` takes `model_id` as a direct kwarg. Today ran loose by the builder's call; strict testing starts Monday |
| **P8e** | **The agent bets properly** (builder's gap review, 2026-08-14) | ✅ done — the seven gaps the builder named, closed in one increment. **Brief v2** (`poc_reasoning.assemble_brief` + `gold_poc_data.market_context`): the target's identity in units, its last 10 daily % moves, every related market's 1-/5-session move (silver, USD/RUB, USD/INR, WTI, dollar index, VIX, real-10y — FRED knowledge-lag respected; moves only, never levels), the live record with realised % beside each outcome **and the agent's own past point calls graded**, plus a rewritten task: *the numeric methods are evidence, not a menu — make your own forecast*. **Point bets**: `point_pct` per horizon (required, bounded ±25) sealed in the llm metadata; the distribution stays the RPS-scored contract so the exact call can never be gamed into the headline. **All five instruments** now run the reasoning pair and keep their own wiki page (curator per instrument at maturation; registry-format offline results parse for seeding). **Dashboard**: page re-zoned (Today / live / offline) with a jump bar; "The agent, bet by bet" board shows model bet → actual in price space via a local-only sidecar (`ui/data/live_prices.js`, excluded by the ui/data allowlist — committed files stay derived-work-only). REQ-1301 amended in place. ~10 predict calls/day + up to 5 curator calls |
| **P8f** | **First strict day's review** (2026-08-17, builder's request) | ✅ done — the 11:20 catch-up run was mechanically clean (all 6 t+1 outcomes recomputed independently and matched; 3 instruments sealed 08-15 with the reasoning pair; USD/RUB's `llm_raw` was that day's best method — one day). **One real defect found and fixed:** the curator's first three lessons all read *"when ≥2 Fighting events, expect no fall"* — but Fighting appeared on **7 of 7** shortlist days, so the condition was universal and the lesson a two-day coincidence in a rule's clothing. The evidence digest now leads with each label's **base rate across the window**, attributes events to the **anchor day** (what was knowable when the bet was placed) rather than the target day, and the curator prompt requires conditions the base rates show to be rare — with an explicit instruction to retire the existing near-universal lessons. **Then made code, not prompt (builder's direction, same day):** `apply_curation` drops any lesson naming an event label present on >70% of the window's days (`UNIVERSAL_LABEL_SHARE`), and applies the same rule to *existing* lessons on a proposed-nothing day so the keep-existing rule cannot resurrect one; dry-run against today's real files retires all three (Assault, Coercion, Fighting, Threat are all >70% this window). 4 tests, one replaying the committed wiki against the committed events. **The point-call bar (builder's challenge, same day — "a trader can't act on a 100 ₽ miss"):** measured, gold moves a median 1.1%/day (~130 ₽/g) — the floor no point forecaster gets under; the agent board now shows every stat against its bar on the same graded days: exact-bucket ticks vs *always-flat*, average miss vs *always 0%*, and RPS wins vs every other method, with a plain-English verdict per instrument. Day one: gold's calls missed by more than saying "no change" and tied always-flat; USD/RUB's beat both bars — 2 days, not a finding. The nowcast lever (a live spot quote at run time; the D+1 fix is set from day-D London trading) is scoped as the next accuracy increment, pending the builder's data-source call | **Two clarity gaps closed:** the brief now says plainly when GDELT's newest published day predates the seal (a ~1.5-day upstream lag verified against GDELT's index — *not yet available*, never *quiet*); the dashboard opens on the horizon that has grades, every "waiting" cell says which session grades it, and the curator's page is finally on the dashboard ("Its memory") so a bad lesson is visible rather than hidden. Nothing resealed; nothing scored touched |
| **P9** | **Controls for the reasoning rung** (L1, builder's go 2026-08-19) | ✅ done — [ADR-0058](adr/0058-controls-for-the-reasoning-rung.md) + REQ-1304. `llm_noevents` isolates the events→prices thesis, `llm_blind` measures anchoring (ADR-0029, unbuilt until now), `momentum_climo` is a free deterministic trend bar (`drifted_climatology`: climatology with the recent per-session drift added to every historical return in σ units; zero drift reproduces climatology exactly). **Purely additive** — `llm_raw`'s brief is byte-identical and a test pins it, so nothing already accruing changes. Each control removes exactly one input and its task text never points at what was removed (a dangling reference is a different prompt, not a clean removal — caught by its own test). The primary comparison and its ~60-day minimum are pre-registered in the ADR *before* the first control seals; controls start their own clock and are never backfilled. First seals at the 2026-08-20 run |
| **P8g** | **The review after two strict runs + the customer dashboard** (builder's request, 2026-08-18) | ✅ done — [docs/analysis/poc-review-2026-08-18.md](analysis/poc-review-2026-08-18.md). Ten-role review (record analyst + skeptic recompute; methodology auditor + adversarial refuter; commercial strategist; three designers, a judge, a builder). **Performance:** every outcome, score and hash recomputed twice to zero difference; on gold's two agent-graded days llm_mem 0.0911 / climatology 0.0911 / llm_raw 0.0934 — indistinguishable; the agent's point call ties always-0% exactly; memory −0.001 RPS over 8 pairs with the page at 0 lessons — nothing learned, nothing lost, expected on day five. **Method:** 18 findings, all upheld; five defects (D1–D5) recommended for immediate fix despite the freeze, levers L1–L7 queued behind 08-24 under a pre-registered decision rule (NW-effective SD 0.03–0.06 t+1 / 0.04–0.13 t+5 → MDD ~0.03–0.05 at 5 days, ~0.01 at 60: the week certifies plumbing only). **Commercial:** the asset is the evaluation discipline — a forecast audit / trust layer for any source; the smallest saleable increment is an external-source adaptor (C1). **Customer dashboard:** `ui/customer.html`, "the range ledger — sealed before, graded after", eight sections, verified independently at 1280 and phone widths and in the public build (no price leak); linked both ways with the technical page |
