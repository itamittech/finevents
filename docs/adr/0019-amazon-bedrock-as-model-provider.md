...
>
> **Second amendment note (added on review): every model reference below is stale.** This ADR defaults all stages to **Claude Opus 5** via `AnthropicBedrockMantle` with an `anthropic.` model-ID prefix. The chain 0022 → 0027 → [0040](0040-split-reasoning-model-by-role.md) replaced that entirely: all three roles are now **`amazon.nova-*`** — Lite classifies, Pro predicts, Premier curates — resolved from SSM per role, so no model ID appears in code at all (REQ-1007). The `AnthropicBedrockMantle` client and the `anthropic.` prefix apply to Anthropic models and are **not** the path this project uses today. **The Bedrock decision stands; the model, the client and the prefix below do not.** The prompt-cache observation remains valid in shape but not in numbers — Nova caps cached content near 20k tokens, which is a different constraint from the Opus/Haiku minimums quoted below (REQ-1014 requires re-deriving these on every model change).
