...
### 2a. Defects — ✅ ALL FIVE FIXED 2026-08-19 (builder's go; the freeze otherwise stands)

Each was a correctness bug rather than a tuning, so fixing them mid-week makes the
observation week measure the configuration it was always meant to measure. Nothing was
resealed and no sealed number changed: the record is append-only, and the rows already
taken by the leaked briefs are **shown but excluded** from the agent's counts on both
dashboards, marked "⚠ pre-fix context" (usd_inr 08-07 and 08-14, wti 08-11). Seals from
the fix onward carry `context_through` and `events_through`, so the information set
behind every future bet is visible in the record itself and the exclusion becomes
automatic rather than a list. 50 POC tests (5 new), boundaries and traceability clean.

| # | Finding | Why it matters | ✅ What was done |
|---|---|---|---|
| D1 | **Knowledge-time leak for the FRED-lagged instruments.** `market_context()` takes no as-of: for USD/INR and WTI (whose own series lag the wall clock by days) the brief carried related-market moves and events dated *after* the anchor — for USD/INR as-of 08-14, gold moves through 08-18 and events through 08-17, i.e. after its own t+1 target. Gold/silver/USD-RUB are unaffected (their as-of *is* the run day). | Those instruments' agent grades are not honest. Every further seal adds a contaminated row. | `market_context(instrument, as_of=…)` cuts **every** roster series at the anchor (`_cut`), and `assemble_brief` cuts the event shortlist the same way. Verified on the real USD/INR brief: related markets now stop at 2026-08-14 where they ran to 08-18, and events start at the anchor instead of 08-17. Both dashboards mark and exclude the four pre-fix rows. |
| D2 | **t+5 point calls graded against the wrong move inside the brief.** `actuals` is a map of *daily* moves keyed by date; the brief looks up `actuals[target_date]` for t+5 too, telling the model "you called +0.16%, it moved −0.01%" when the 5-session move was +0.23%. The dashboard computes t+5 correctly; only the brief is wrong. | Gold's first t+5 grades land 08-20 — from Wednesday the gold brief would grade the model's own calls falsely. | `horizon_moves[h][anchor]` replaces the daily-move map, so the brief now reads "realised 'small up' (+1.40% over 5 session(s))". Recomputed against the source: anchor 2026-08-07 t+5 = **+0.231%**, exactly the figure the brief had been reporting as −0.01%. A test pins t+1 and t+5 apart. |
| D3 | **A partial CBR fetch could seal a backdated record.** `fetch_metals_history.py` continues past a failed 365-day chunk and rewrites the CSV with whatever survived; if only the newest chunk fails, `as_of = dates[-1]` regresses (e.g. to 2026-01-08), no existing record matches, and the runner seals — and calls the model — "as of" a date months back. Documented as "halts on failure"; the code does the opposite. | A single bad morning could put a fabricated-date seal into a seal-once record. | Three layers: a failed CBR chunk now raises instead of continuing; `write()` refuses to replace a CSV with one that ends earlier or holds fewer rows; and `guard_forward()` halts the runner before any seal or model call if an instrument's series ends before a date already sealed. |
| D4 | **A mid-run crash loses paid bets and overwrites their only transcript.** `live.js` is written once at the end; the model calls happen per instrument earlier; `record_run()` writes transcripts to a fixed path. Any exception after gold's calls (HF Hub, a malformed FRED CSV, the 2-hour task limit) discards that day's bets, and the re-run re-calls a nondeterministic model and overwrites the transcript — the published hashes then match the second attempt. | Auditability is the whole honesty mechanism of the reasoning rung. | `save_progress()` writes the record, the wiki and the sidecar after every instrument; `record_run()` never overwrites — a second attempt writes `…_attempt2.json`, and the seal records `transcript` (and `attempt`), so the first bet's evidence survives a crash and a re-run. |
| D5 | **The curator cannot retire its last lesson.** An explicit `lessons: []` is treated as "proposed nothing — keep existing". On 08-18 the curator *did* propose retiring the Fighting lesson; the code ignored that and the guard did the retiring. Right outcome, wrong reason. | The one path to honest forgetting is closed. | An explicit `lessons: []` now empties the page, and every dropped lesson is logged as `retired: …`. "Keep existing" belongs to the runner's exception branch, where a *failed* curator call already keeps the prior page — silence and retirement are no longer the same event. |
...
| L4 ✅ | Offline FRED knowledge lag (+1 day) understates the real publication lag of the weekly/lagged series (H.10, EIA), and even the dailies at the run hour | Per-series publication lag in the loader; re-run the offline ladder; the rwcov control stays the live judge — **done 2026-08-19**: measured per-series bounds (dailies 2 business days; H.10 up to 8 calendar days; EIA 9) plus a committed first-seen ledger that makes them exact going forward. The panel takes knowledge-dated series, the brief value-dated series filtered by knowledge, because a weekly release publishes five days at once and re-dating would collapse the shape. Old vs new join differed on 91–100% of report-window sessions; all five offline ladders re-run. |
...
|---|---|---|---|
| **R1** | Defects D1–D5 (plus H1's `context_through` / `events_through`, which D1 needed) | D1/D2 change the brief only where it was *wrong*; the rest is additive | ✅ **done 2026-08-19.** Hygiene H2, H3, H4, H7, H8 remain, batched with R5 |
