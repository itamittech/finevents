...
**Traces to:** [Product.md](Product.md), [SystemDesign.md](SystemDesign.md), [ADRs 0001–0056](adr/README.md)
...
| REQ-504 | Each model runs in two configurations — univariate and covariate-informed — over 11 instruments, giving 44 forecasts per day. | ADR-0030, 0031, 0055, 0056 | `I` |
...
| ID | Requirement | Traces to | Verify |
|---|---|---|---|
| REQ-1301 | The reasoning rungs run on the Strands framework with **provider and model taken from the environment, never code**, and emit the same five-bucket contract as every rung — sealed before outcomes exist and matured against sealed edges, exactly as REQ-1303's record requires. A run that cannot produce the contract seals nothing that day, visibly. From P8e the rungs run for **every POC instrument**, the brief carries the market's own context (the target's recent moves, related-market moves, the agent's graded past calls — moves only, never raw levels), and each bet includes a **point estimate** (signed percent move per horizon) sealed beside the distribution; the distribution remains the sole RPS-scored contract. | ADR-0057 | `I` |
...
| REQ-1304 | Every reasoning arm ships with **controls sealed on the same live days**, extending ADR-0056's principle to the reasoning layer: `llm_noevents` (the brief minus the event block) isolates the events→prices thesis, `llm_blind` (minus the other methods' bets, the live record and the offline ladder) measures anchoring, and the deterministic `momentum_climo` (climatology with the recent per-session drift added to every historical return, in σ units) is a free bar for trend-following. The primary comparison, its estimator and its minimum sample are **pre-registered before the first control seals**; controls start their own clock and are never backfilled. | ADR-0058, ADR-0056, ADR-0029 | `I` |
